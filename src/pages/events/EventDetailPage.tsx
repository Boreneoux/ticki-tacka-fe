...
import { formatDate, formatShortDate, formatTime } from '@/features/events/helpers';
...
export default function EventDetailPage() {
  ...
  const { event, ticketTypes: apiTicketTypes, isLoading, error } = useEventDetail(slug);
  const {
    reviews,
  ...
  const handleTicketQty = (ticketId: string, delta: number) => {
  ...
  const handleSubmitReview = async (payload: { rating: number; reviewText?: string }) => {
    if (!isLogin) {
                ...
                <CardHeader>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      {eventImages.length === 0 && event.category && (
                    ...
                    <div className="flex items-start gap-3 p-3 rounded-lg bg-muted/50">
                      ...
                      <div className="min-w-0">
                        ...
                        {event.city && (
                          <p className="text-sm text-muted-foreground">
                            {event.city.name}
                            {event.city.province && `, ${event.city.province.name}`}
                          </p>
                        )}
                      </div>
                    </div>
            ...
            <div className="lg:col-span-1">
              <div className="sticky top-24 space-y-6">
                <TicketTypesList
                  ...
                  onTicketQtyChange={handleTicketQty}
